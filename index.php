<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CampusConnect</title>
    <link rel="stylesheet" href="style.css">
</head>

<body class="cc-body">

    <!-- NAVBAR -->
    <nav class="cc-navbar">
        <div class="cc-logo-section">
            <img src="/Assets/Logo.png" class="cc-logo-icon" alt="logo">
            <span class="cc-logo-text">SmartCampus</span>
        </div>

        <ul class="cc-nav-links">
            <li><a href="" class="cc-active-link">Home</a></li>
            <li><a href="/about.html">About</a></li>
            <li><a href="/contact.html">Contact Us</a></li>
        </ul>
    </nav>

    <!-- MAIN SECTION -->
    <section class="cc-main-section">
        <div class="cc-text-box">
            <h1 class="cc-title">SmartCampus</h1>
            <p class="cc-subtitle">College Communication & Management System
            <p class="cc-desc">
                A platform designed to enhance communication between the college 
                administration, faculty, and students.
            </p>

            <a  href="/login.html" class="cc-start-btn">Get Started </a></button>
        </div>

        <div class="cc-image-box">
            <img src="/Assets/BackgroundHome.png" class="cc-main-image" alt="illustration">
        </div>
    </section>

</body>
</html>
